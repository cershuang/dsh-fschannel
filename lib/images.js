...
export class HeldImageBuffer {
   ...
   *
   * These files ARE in the durable image index — the docstring used to claim
   * otherwise, but ingestImages indexes every successfully staged file. The
   * caller must therefore drop the index entry as well as the file, or the
   * gallery keeps listing an image whose bytes are gone.
   * @param {number} ttlMs
   * @returns {Array<{ chatId: string, path: string, name: string }>} the pruned
   *   files, for the caller to delete and de-index (best effort).
   */
  prune(ttlMs) {
    if (ttlMs <= 0) return []
    const pruned = []
    ...
    for (const [chatId, entry] of this.entries) {
      if (entry.receivedAt < cutoff) {
        this.entries.delete(chatId)
        for (const file of entry.files) pruned.push({ chatId, path: file.path, name: file.name })
      }
    }
    return pruned
  }
