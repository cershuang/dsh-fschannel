...
export function apply(ctx, config) {
  ...
  const ingestImages = async (msg, record) => {
    ...
    for (const resource of images) {
      ...
      if (result.file !== undefined) {
        ...
        held += 1
        const evicted = store.setImage(record.sessionId, basename(result.file.path), result.file.path)
        for (const path of evicted) {
          try { rmSync(path, { force: true }) } catch { /* best effort */ }
        }
        appendImageEvent(record, result.file)
  ...
  /** Image events queued while their session's agent was not live. */
  /** @type {Array<{ sessionId: string, data: object }>} */
  const pendingImageEvents = []
  /** Cap on queued image events; beyond it the oldest entries are dropped. */
  const MAX_PENDING_IMAGE_EVENTS = 200

  /**
   * Append a log-only session event so the Web client renders the staged image
   * as a chat node. The event carries only metadata (never image bytes); it has
   * no surfaceOp, so it never enters the LLM history and never wakes the agent.
   * When the agent is not live, the event is queued and flushed by the next
   * delivery (deliver() flushes pending image events after acquiring the agent).
   * @param {BindingRecord} record
   * @param {import('./images.js').HeldImageFile} file
   */
  const appendImageEvent = (record, file) => {
    ...
    const queue = () => {
      pendingImageEvents.push({ sessionId: record.sessionId, data: eventData })
      if (pendingImageEvents.length > MAX_PENDING_IMAGE_EVENTS) {
        pendingImageEvents.splice(0, pendingImageEvents.length - MAX_PENDING_IMAGE_EVENTS)
      }
    }
    try {
      const session = ctx.agents.get(record.sessionId)?.session
      if (session === undefined) {
        queue()
        return
      }
      session.append('feishu/image', eventData)
    } catch (error) {
      ctx.logger.warn('feishu: append image event failed: %s', error instanceof Error ? error.message : String(error))
      queue()
    }
  }
  ...
  const deliver = async (record, text, replyTo) => {
    ...
    // Flush image events queued while this session's agent was offline.
    if (pendingImageEvents.length > 0) {
      const queued = pendingImageEvents.filter((entry) => entry.sessionId === record.sessionId)
      if (queued.length > 0) {
        for (const entry of queued) pendingImageEvents.splice(pendingImageEvents.indexOf(entry), 1)
        for (const entry of queued) {
          try {
            ctx.agents.get(record.sessionId)?.session?.append('feishu/image', entry.data)
          } catch (error) {
            ctx.logger.warn('feishu: flush image event failed: %s', error instanceof Error ? error.message : String(error))
          }
        }
      }
    }
