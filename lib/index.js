...
export function apply(ctx, config) {
  ...
  /** @type {Map<string, { chatId: string, feishuMessageId: string }>} dsh message id -> chat + Feishu message */
  ...
  const readModels = async (sessionId) => {
    ...
    try {
      ...
      ctx.logger.warn('feishu: model directory read failed for %s: %s', sessionId, error instanceof Error ? error.message : String(error))
      return undefined
    }
  }
