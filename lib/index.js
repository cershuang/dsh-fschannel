...
import { composeImageNote, extFor, HeldImageBuffer, validateImage } from './images.js'
 ...
 *   output?: 'stream' | 'plain', modelCardTriggers?: boolean,
...
export function apply(ctx, config) {
  ...
  const maxHeldImages = typeof config.maxHeldImages === 'number' && config.maxHeldImages > 0 ? config.maxHeldImages : 10
  const maxHeldImageBytes = typeof config.maxHeldImageBytes === 'number' && config.maxHeldImageBytes > 0 ? config.maxHeldImageBytes : 10 * 1024 * 1024
  const holdTtlMs = typeof config.holdTtlMs === 'number' && config.holdTtlMs >= 0 ? config.holdTtlMs : 0
  ...
  const handleMessage = async (msg) => {
    ...
    if (record !== undefined) {
      ...
      // Images: stage them now; combine with the text when this (or a later)
      // message carries one. Image-only messages never wake the agent.
      ...
      if (held.length > 0) {
        const combined = (ingested.text ?? '').trim() === ''
      ...
      if ((ingested.text ?? '').trim() === '') {
        // Image-only message with nothing combined: acknowledge, do not process.
        if (holdHint && ingested.held > 0) {
          await send(msg.chatId, `已收到 ${ingested.held} 张图片，发送文字说明后将一起识别处理。`, msg.messageId)
        }
        return
      }
      await deliver(record, ingested.text, msg.messageId)
      return
    }
