...
export function apply(ctx, config) {
  ...
  const handleModelCommand = async (record, args, replyTo) => {
    ...
    if (sub === 'list' || sub === 'ls') {
      ...
      if (directory === undefined) {
        await send(record.chatId, '⚠️ 无法读取模型目录（会话不可用）', replyTo)
        return
      }
      const lines = ['📚 模型目录：']
    ...
    if (sub === 'use') {
      ...
      if (provider === undefined || provider === '') {
        await send(record.chatId, '⚠️ 无法确定提供方，请用完整路径：/model use <provider>/<model>', replyTo)
        return
      }
    ...
    if (sub === 'effort') {
      ...
      if (directory === undefined || directory.current === undefined) {
        await send(record.chatId, '⚠️ 无法读取当前模型（会话不可用）', replyTo)
        return
      }
  ...
  const deliver = async (record, text, replyTo) => {
    ...
    if (agent === undefined || !active) {
      await send(record.chatId, '⚠️ 会话不可用，请检查该会话是否仍存在。', replyTo)
      return
    }
      ...
      if (line.startsWith('/')) {
        ...
        if (command === 'help') {
          ...
          if (args.length === 0) {
            ...
            if (card === undefined) {
              await send(msg.chatId, '⚠️ 无法读取当前模型（会话不可用）', msg.messageId)
            } else {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    switch (event?.type) {
      case 'assistant/chunk': {
        ...
        entry.empty = false
        entry.handle.append(chunk.text)
        break
      }
      case 'tool/call': {
        if (output !== 'stream') break
      ...
      case 'turn/end': {
        ...
        if (entry !== undefined) {
          ...
          if (entry.empty && event.data?.reason?.kind !== 'error') {
            void send(record.chatId, '（本轮没有产生输出）', replyTo).catch(fail)
          }
          if (event.data?.reason?.kind === 'error') {
            const detail = event.data?.reason?.message ?? event.data?.reason?.code ?? ''
            entry.handle.append(`\n\n⚠️ 本轮任务失败${detail === '' ? '' : `：${detail}`}`)
          }
          void entry.handle.finish().catch(fail)
        } else if (event.data?.reason?.kind === 'error') {
          const detail = event.data?.reason?.message ?? event.data?.reason?.code ?? ''
          void send(record.chatId, `⚠️ 本轮任务失败${detail === '' ? '' : `：${detail}`}`, replyTo).catch(fail)
        }
        break
      }
      default:
