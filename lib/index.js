...
import { renderPlainCard, renderFinalCardWithImages } from './render.js'
...
import { createSendImageTool } from './send-image-tool.js'
...
/** Plugin version, read from this package's manifest for the status surface. */
 ...
 *   maxHeldImageBytes?: number, holdTtlMs?: number, imageDir?: string,
 *   sendImageTool?: boolean, sendFileTool?: boolean }} FeishuConfig
 */
...
export function apply(ctx, config) {
  ...
  // Persisted settings (from the settings page) override the entry config.
  {
    ...
    // No value guard: the other four keys below apply unconditionally, and so
    // must these. Testing for 'plain'/false meant a persisted 'stream' could
    // never override an entry config of 'plain' — the user flipped the toggle
    // in the GUI, restarted, and it flipped back.
    if (store.settingsHas('output')) output = persisted.output
    if (store.settingsHas('showImages')) showImages = persisted.showImages
  ...
  ctx.on('session/event', (session, event) => {
    ...
    // Embedding hooks for the final card: upload bytes to Feishu and map a
    // markdown image src to a readable local path inside the session
    // workspace. Remote URLs are not embeddable and stay as plain text.
    const larkChannel = channel // non-null: guarded above; stable for this handler run
    const uploadImage = async (buffer) => {
      const response = await larkChannel.rawClient.im.v1.image.create({
        data: { image_type: 'message', image: buffer },
      })
      const record = /** @type {{ image_key?: string, data?: { image_key?: string } } | undefined} */ (response)
      const key = record?.image_key ?? record?.data?.image_key
      if (typeof key !== 'string' || key === '') throw new Error('image_key missing in upload response')
      return key
    }
    const cwdForImages = sessionCwdOf(session.id)
    const resolveSrc = (src) => {
      const raw = typeof src === 'string' && src !== '' ? src : ''
      if (raw === '') return undefined
      // Remote URLs (http/https/data) are never local files — not embeddable.
      if (/^(https?:|data:)/i.test(raw)) return undefined
      // Confine to the session workspace (same rule as the image route and the
      // send tools): absolute paths must live under the cwd, relative paths
      // resolve against it, and `..`/cross-drive escapes are rejected.
      if (cwdForImages === undefined) return undefined
      const abs = isAbsolute(raw) ? raw : resolve(cwdForImages, raw)
      const rel = relative(cwdForImages, abs)
      if (rel === '' || rel.startsWith('..') || rel.includes('..\\') || rel.includes('../') || isAbsolute(rel)) {
        return undefined
      }
      return abs
    }
    const embedImages = (text) => renderFinalCardWithImages(text, { uploadImage, resolveSrc })
        ...
        if (entry !== undefined) {
          ...
          void entry.handle.finish().then(async () => {
            ...
            if (messageId === undefined) {
              ...
              const { card } = await embedImages(finalText)
              if (card !== undefined) {
                try {
            ...
            try {
              ...
              try {
                ...
                const fallback = await embedImages(finalText)
                if (fallback.card !== undefined) {
                  try { await sendCard(record.chatId, fallback.card, replyTo) } catch { /* best effort */ }
                }
              }
            }
          }).catch(fail)
   ...
   * could repoint the bot at an attacker-controlled app. A request must
   * therefore carry either no Origin (curl, scripts, the ops endpoints) or one
   * that is genuinely this server. `Origin: null` — sandboxed iframes, file://
   * pages — is not a same origin and is rejected.
   *
   * Origin is validated against the socket's LOCAL PORT, never against the Host
   * header. Host is supplied by the caller, so comparing the two compares two
   * attacker-controlled values to each other: a page at http://evil.example
   * whose DNS is rebound to 127.0.0.1 connects over loopback and sends a
   * matching Origin/Host pair. The browser then treats it as same-origin, so it
   * may set application/json without a preflight — clearing the second barrier
   * — and read the response, which carries credential status. The local port is
   * the one value in the exchange the caller cannot forge.
   *
   * The port must match too. Stripping it would admit any other loopback origin
   * — a dev server on :3000, a local app's HTTP surface, a hosts alias — as
   * same-origin for a credential write.
  ...
  const isSameOrigin = (req) => {
    ...
    // `new URL` strips the brackets from an IPv6 literal.
    const hostname = url.hostname
    if (hostname !== '127.0.0.1' && hostname !== 'localhost' && hostname !== '::1') return false
    const port = url.port !== '' ? url.port : url.protocol === 'https:' ? '443' : '80'
    const localPort = req.socket?.localPort
    return localPort !== undefined && port === String(localPort)
  }
  ...
  // Agent-facing tool: send an image the agent generated to the bound chat.
  // Registered only when the tools service is present (older hosts degrade
  // gracefully) and the feature is enabled.
  ...
  const sendImageToolEnabled = config.sendImageTool !== false
  if (tools !== undefined && sendImageToolEnabled) {
    const definition = createSendImageTool({
      store,
      channel: () => channel,
      sessionCwd: sessionCwdOf,
      indexImage,
      ui: () => ui,
      log: (line) => ctx.logger.warn(line),
    })
    const dispose = tools.register(definition)
    ctx.effect(() => dispose, 'dsh-fschannel: send_feishu_image tool')
  }
  const sendFileToolEnabled = config.sendFileTool !== false
  if (tools !== undefined && sendFileToolEnabled) {
    const definition = createSendFileTool({
      store,
      channel: () => channel,
      sessionCwd: sessionCwdOf,
      ui: () => ui,
    ...
    const dispose = tools.register(definition)
    ctx.effect(() => dispose, 'dsh-fschannel: send_feishu_file tool')
  }
}
