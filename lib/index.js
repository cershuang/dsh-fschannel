 ...
 *   POST /feishu/repair-logs — force a session-log repair pass (rate-limited)
 ...
 * Feishu message parameter throughout this module and would shadow it.
 * @param {unknown} error
 * @returns {string}
 */
      ...
      if (path === '/feishu/config' && req.method === 'POST') {
        ...
        if (typeof body.appId === 'string' && body.appId.trim() !== '') {
          try {
            ...
            json(res, 400, { ok: false, error: `appId 保存失败：${errText(error)}` })
            return
          }
        }
        if (typeof body.appSecret === 'string' && body.appSecret.trim() !== '') {
          try {
            ...
            json(res, 400, { ok: false, error: `appSecret 保存失败：${errText(error)}` })
            return
          }
        }
