...
import { composeImageNote, contentTypeFor, extFor, HeldImageBuffer, textWithoutImageMarkup, validateImage } from './images.js'
...
export function apply(ctx, config) {
   ...
   * history on every boot is wasted work). `force` (the manual endpoint)
   * bypasses the fingerprint.
  ...
  const repairHistoricalLogs = (options) => {
    ...
    if (repaired > 0 || failed > 0 || skipped > 0) {
      ctx.logger.info('feishu: log repair pass finished (%d repaired, %d skipped, %d unchanged)', repaired, failed, skipped)
    }
  }
  ...
  const ingestImages = async (msg, record) => {
    ...
    heldImages.prune(holdTtlMs).forEach((path) => {
  ...
  const openStream = (chatId, opts, onFailure) => new StreamHandle(channel, chatId, opts, onFailure)
  ...
  const applySelection = async (record, selection) => {
    ...
    if (res.result?.ok !== true) {
      const message = res.result?.error?.message ?? ui.modelSwitchFailed.replace('{message}', '')
  ...
  const handleStatusCommand = async (record, replyTo) => {
    ...
    const modelLine = directory?.current === undefined
      ...
      : fill(ui.statusModel, { model: `${directory.current.provider}/${directory.current.model}（effort ${effort}）` })
    const lines = [
      ui.statusTitle,
      ...
      fill(ui.statusSession, {
        ...
        chat: record.chatName === undefined ? '' : `（${record.chatName}）`,
      }),
      modelLine,
    ]
    await send(record.chatId, lines.join('\n'), replyTo)
      ...
      if (line.startsWith('/')) {
        ...
        if (command === 'help') {
          ...
          if (args.length === 0) {
            ...
            if (card === undefined) {
              await send(msg.chatId, ui.modelUnavailable, msg.messageId)
            } else {
    ...
    switch (event?.type) {
      ...
      case 'turn/end': {
        ...
        if (entry !== undefined) {
          ...
          if (reasonKind === 'error') {
            const failed = fill(ui.taskFailed, { detail: detail === '' ? '' : `：${detail}` })
          ...
          const detail = event.data?.reason?.message ?? event.data?.reason?.code ?? ''
          void send(record.chatId, fill(ui.taskFailed, { detail: detail === '' ? '' : `：${detail}` }), replyTo).catch(fail)
        }
        break
      }
      default:
  ...
  const handleRoute = async (req, res) => {    if (!isLoopback(req)) {
      ...
      if (path === '/feishu/config' && req.method === 'POST') {
        ...
        if (body.locale !== undefined) {
          if (body.locale !== 'zh' && body.locale !== 'en') {
            json(res, 400, { ok: false, error: 'locale 必须是 zh 或 en' })
            return
          }
          store.setSettings({ locale: body.locale })
          ui = dictFor(body.locale)
          if (configuredHint === undefined) hintText = defaultHint(ui)
        }
