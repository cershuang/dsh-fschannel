...
function expandRecord(record) {
  ...
  if (tag !== 'text-chunks' && tag !== 'reasoning-chunks' && tag !== 'tool-call-chunks') {
    // A record with no `type` is not an event the seq walk can account for;
    // returning it as `{ type: undefined }` silently corrupted the count.
    if (typeof tag !== 'string') return []
    return [{ type: tag, seq: record.seq, ...(typeof record.data?.message?.id === 'string' ? { id: record.data.message.id } : {}) }]
  }
