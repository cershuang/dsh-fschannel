...
export class BindingStore {
  ...
  constructor(file, log) {
    ...
    this.byChat = new Map()
    /** @type {string[]} pending session ids, FIFO */
    this.pending = []
  ...
  load() {
    try {
      ...
      if (data && Array.isArray(data.pending)) {
        this.pending = data.pending.filter((id) => typeof id === 'string')
      }
  ...
  addPending(sessionId) {
    if (this.bySession.has(sessionId) || this.pending.includes(sessionId)) return false
  ...
  removePending(sessionId) {
    const index = this.pending.indexOf(sessionId)
   ...
   */
  shiftPending() {
    const sessionId = this.pending.shift()
    if (sessionId !== undefined) this.save()
    return sessionId
  }
