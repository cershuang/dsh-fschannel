...
export class BindingStore {
  ...
  bind(sessionId, chatId, chatName, lastInboundMessageId) {
    ...
    const record = {
      ...
      chatId,
      boundAt: Date.now(),
      // Carry the session's model route across a re-bind. The route is chosen
      // per session (/model use), not per chat, so moving a session to another
      // chat must not reset it — and dropping it here made replayModelRoute a
      // guaranteed no-op on every bind path, silently returning the session to
      // the default model. A DIFFERENT session displacing this chat is another
      // matter: that record is deleted whole above, and its route goes with it.
