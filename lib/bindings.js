...
export class BindingStore {
  ...
  load() {
    try {
      ...
      // A missing file is the normal first boot; an unreadable or malformed
      // one means every binding just became invisible, and the next save would
      // overwrite it with an empty document. Those two must not look alike in
      // the log, and the second must not destroy the evidence: quarantine the
      // file so an operator can recover from it.
      if (error !== null && typeof error === 'object' && /** @type {{ code?: string }} */ (error).code === 'ENOENT') {
      ...
      try {
        ...
        this.log(`feishu: UNREADABLE binding store at ${this.file} (${detail}); moved to ${quarantine}, starting empty`)
      } catch (renameError) {
        const renameDetail = renameError instanceof Error ? renameError.message : String(renameError)
        this.log(`feishu: UNREADABLE binding store at ${this.file} (${detail}); could not quarantine it (${renameDetail}) — the next save WILL overwrite it`)
      }
    }
  }
