 ...
 */
export function buildModelCard({ sessionId, current, groups }) {
  ...
  const effortLabel = current.reasoningEffort === undefined ? '默认' : current.reasoningEffort
  ...
  return {
    ...
    header: { template: 'blue', title: { tag: 'plain_text', content: '模型与推理设置' } },
    elements: [
      {
        tag: 'div',
        text: {
          tag: 'lark_md',
          content: `**当前**：\`${current.provider}/${current.model}\` · effort \`${effortLabel}\``,
        },
      },
      {
        tag: 'action',
      ...
      { tag: 'note', elements: [{ tag: 'plain_text', content: '点击按钮立即生效，下一条消息起使用。' }] },
    ],
  }
}
