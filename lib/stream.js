...
export class StreamHandle {
  /**
   * @param {{ stream: (chatId: string, spec: object, opts?: object) => Promise<{ messageId?: string } | undefined> }} port -
   *   the outbound transport.
   * @param {string} chatId - the bound chat.
   * @param {object | undefined} opts - send options fixed when the card opens (replyTo...).
   ...
   *   Callers pass the locale copy; the empty default means "no placeholder"
   *   rather than a hard-coded language.
   */
  constructor(port, chatId, opts, onFailure, initialText = '') {
    this.port = port
    this.chatId = chatId
    this.opts = opts
